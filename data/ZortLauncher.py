...
def connect_discord():
    ...
    try:
        ...
        rpc.update(
            details="The Best Minecraft Launcher",
...
def install_themes():
    ...
    # plugin_dir içindekileri sil
    if os.path.exists(plugin_dir):
        for item in os.listdir(plugin_dir):
            item_path = os.path.join(plugin_dir, item)
            try:
                if os.path.isfile(item_path) or os.path.islink(item_path):
                    os.unlink(item_path)  # dosya veya link sil
                elif os.path.isdir(item_path):
                    shutil.rmtree(item_path)  # klasör sil
            except Exception as e:
                print(f"Hata oluştu: {e}")
                QMessageBox.warning(None, "Hata", f"{item_path} silinirken hata oluştu:\n{e}")

    else:
        os.makedirs(plugin_dir)  # plugin_dir yoksa oluştur

    # Kullanıcıdan zip dosyası seçmesini iste
    options = QFileDialog.Options()
    options |= QFileDialog.ReadOnly
    zip_file_path, _ = QFileDialog.getOpenFileName(
        None, "Tema ZIP dosyası seç", "", "ZIP Files (*.zip);;All Files (*)", options=options)

    if not zip_file_path:
        return  # kullanıcı iptal etti

    # Zip dosyasını plugin_dir içine aç
    try:
        with zipfile.ZipFile(zip_file_path, 'r') as zip_ref:
            zip_ref.extractall(plugin_dir)
        QMessageBox.information(None, "Başarılı", "Temalar yüklendi! Uygulamayı Yeniden Başlattıüınızda Tema Değişir.")
    except Exception as e:
        QMessageBox.critical(None, "Hata", f"Zip dosyası açılırken hata oluştu:\n{e}")
...
class Ui_MainWindow(object):
    ...
    def run_minecraft(self):
        ...
        if version:
            options = {
                ...
                'token': access_token,
                'jvmArguments': arg,
                'launcherName': "ZortLauncher for Minecraft",
    ...
    def settings_window(self):
        ...
        def set_lang():
            ...
            bt_open_themes.setText(lang(new_lang, "" \
            ""))
            bt_install_themes.setText(lang(new_lang, "install_themes"))
        ...
        bt_install_themes = QPushButton(lang(system_lang, "install_themes"))
        bt_install_themes.setFixedSize(480, 30)
        bt_install_themes.setStyleSheet(self.bt_style)
        bt_install_themes.clicked.connect(install_themes)
        layout.addWidget(bt_install_themes)
